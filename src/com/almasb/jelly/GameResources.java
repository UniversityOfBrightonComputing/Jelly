...
/**
 * Loads and holds all the game resources Can be accessed to retrieve specific
 * res
 *
 * @author Almas
 * @version 1.0
...
public class GameResources {
    ...
    /**
     * Loads all necessary in-game resources such as images and sounds
     */
    static {
        /* LOADING IMAGES */
        ...
        for (int i = 0; i < IMG_ANIMATION_PLAYER.length; i++)
            IMG_ANIMATION_PLAYER[i] = getImg("animation_player/walk" + (i + 1)
                    + ".png");
     ...
     *
     * @param fileName
     *            - full or relative pathName + fileName
    ...
    private static BufferedImage getImg(String file) {
        ...
        try {
            ...
            try {
                fileName = "res/images/" + file;
                return ImageIO.read(GameResources.class.getClassLoader()
    ...
    private static AudioClip getAudioFile(String fileName) {
        try {
            ...
            // return
            // Applet.newAudioClip(Resources.class.getResource(fileName));
        }
        catch (Exception e) {
